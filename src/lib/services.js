...
export async function createComment({post, email, content}) {
  const {data, error} = await supabase
    .from('comments')
    .insert({post, email, content})
  return {data, error}
}
...
export async function getPosts() {
  ...
  data = await Promise.all(data.map(async (post) => {
    const [{count: likes, }, {data: comments}, {publicURL} ] = await Promise.all([
      ...
      post.image ? await supabase
        .storage
        .from('images')
        .getPublicUrl(post.image.split('/').slice(1).join('/')) : Promise.resolve({})
    ])
    return {
