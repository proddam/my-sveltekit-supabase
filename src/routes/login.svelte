  ...
  {:else}
    <form class="form-control" on:submit|preventDefault={handleSignIn}>
      <label for="email" class="label">
        <span class="label-text text-3xl">Login to Qwitter!</span>
      </label>  
      <div class="relative">
        <input bind:value={email} id="email" type="email" placeholder="Email" class="w-full pr-16 input input-primary input-bordered">
